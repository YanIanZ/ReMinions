...
/**
 * Real NMS adapter for Paper 1.21.8 – 1.21.10. Uses the post-1.21.2 SlotDisplay system to
 * render the ghost recipe with custom per-slot lore (input amounts) and the minion-head
 * preview tile.
 */
public final class NMSHandlerImpl_1_21_8 implements NMSHandler {
    ...
    public void sendGhostRecipe(PlayerRecipeBookClickEvent event, Config config, MinionManager minionManager,
        ...
        SlotDisplay.ItemStackSlotDisplay resultDisplay = new SlotDisplay.ItemStackSlotDisplay(nmsResult);
        SlotDisplay.ItemStackSlotDisplay craftingTableDisplay = new SlotDisplay.ItemStackSlotDisplay(
                new net.minecraft.world.item.ItemStack(Items.CRAFTING_TABLE));
        ShapedCraftingRecipeDisplay display = new ShapedCraftingRecipeDisplay(
                shape[0].length(),
                shape.length,
                slots,
                resultDisplay,
                craftingTableDisplay
        );
